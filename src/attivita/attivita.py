...
class attivita_aggiornamenti(osv.Model):
    ...
    _columns = {
        ...
        'attivita_id': fields.many2one('attivita.attivita', 'Attività',required=True),
        'data_creazione': fields.date('Data Creazione', readonly=1, required=True),
    ...
    def create(self, cr, uid, data, context=None):
        aggiornamento_id = super(attivita_aggiornamenti, self).create(cr, uid, data, context=context)
        template_model_data = self.pool.get('ir.model.data').search(cr, uid, [('name', '=', 'template_email_notifica_referente_aggiornamento')])
        if len(template_model_data):
            template_id = self.pool.get('ir.model.data').browse(cr, uid, template_model_data[0])
            self.pool.get('email.template').generate_email(cr, uid, template_id.res_id, aggiornamento_id, context)
    ...
    def segna_letto(self, cr, uid, ids, context=None):
        ...
        if attivita.referente_id.id == uid:
            self.write(cr, uid, ids, {'state': 'letta','data_lettura': time.strftime("%Y-%m-%d")})
        else:
            raise osv.except_osv(_('Attenzione!'), _('Solo il referente del messaggio può marcarlo come Letto'))
        return True
...
class attivita_categoria(osv.Model):
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('attivita.categoria','Categoria Superiore'),
        'descrizione': fields.text('Descrizione'),
        'attiva': fields.boolean('Attiva'),
        'tempo_standard' : fields.integer('Tempo Standard di Realizzazione (in gg)'),
    }
...
class attivita_attivita(osv.Model):
    ...
    def _tempo_realizzazione_calc(self, cr, uid, ids, name, arg, context=None):
        ...
        for attivita in self.browse(cr, uid, ids, context=context):
            if attivita.data_conclusione and attivita.data_assegnazione:
                data_conclusione= datetime.datetime.strptime(attivita.data_conclusione, '%Y-%m-%d')
                data_assegnazione= datetime.datetime.strptime(attivita.data_assegnazione, '%Y-%m-%d')
    ...
    def _tempo_ritardo_calc(self, cr, uid, ids, name, arg, context=None):
        ...
        for attivita in self.browse(cr, uid, ids, context=context):
            if attivita.data_conclusione and attivita.data_scadenza:
                data_conclusione= datetime.datetime.strptime(attivita.data_conclusione, '%Y-%m-%d')
                data_scadenza= datetime.datetime.strptime(attivita.data_scadenza, '%Y-%m-%d')
    ...
    _columns = {
        ...
        'ultimo_aggiornamento': fields.datetime('Data Ultimo Aggiornamento', readonly=True, required=True),
        ...
        'storico_ids': fields.one2many('attivita.storico','attivita_id','Storico'),
        'state': fields.selection(_type_selection,'Stato', readonly=True),
        'tempo_realizzazione': fields.function(_tempo_realizzazione_calc, type='float', string='Tempo Realizzazione',
            help="Calcolato dalla differenza: Data Conclusione - Data Assegnazione", store=True, group_operator="avg"),
        'tempo_ritardo': fields.function(_tempo_ritardo_calc, type='float', string='Tempo Ritardo',
            help="Calcolato dala differenza: Data Conclusione - Data Scadenza", store=True, group_operator="avg"),
...
#        return super(attivita_attivita, self).create(cr, uid, data, context=context)
    ...
    def write(self, cr, uid, ids, data, context=None):
        ...
        if data.has_key('state'):
            storico_data = {'name': self._get_state_definition(cr,uid, data['state']),'attivita_id': attivita_id}
            if data['state'] == 'annullato':
                ...
                if data.has_key('richiesta_integrazione'):
                    storico_data['descrizione'] = data['motivazione_richiesta_integrazione']
                else:
                    assegnatario_name = ''
                    if data.has_key('assegnatario_id'):
                        assegnatario = user_obj.browse(cr,uid,data['assegnatario_id'])
                        if assegnatario:
                            assegnatario_name = assegnatario.name
                    storico_data['descrizione'] = 'Assegnato a: ' + assegnatario_name
            elif data['state'] == 'lavorazione':
                storico_data['descrizione'] = 'Preso in carico da: ' + user_obj.browse(cr,uid,uid).name
            elif data['state'] == 'concluso':
                storico_data['descrizione'] = 'Concluso da: ' + user_obj.browse(cr,uid,uid).name
            elif data['state'] == 'chiuso':
                storico_data['descrizione'] = 'Chiuso da: ' + user_obj.browse(cr,uid,uid).name
    ...
    def valida(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {'state': 'chiuso','data_chiusura': time.strftime("%Y-%m-%d")})
        return True

    def onchange_categoria(self, cr, uid, ids, categoria_id, data_assegnazione, context=None):
        result = {}
        if categoria_id:
            categoria = self.pool.get('attivita.categoria').browse(cr, uid, categoria_id)
            if categoria.tempo_standard:
                data_partenza = data_assegnazione and datetime.datetime.strptime(data_assegnazione, '%Y-%m-%d') or datetime.datetime.today()
                data_scadenza = data_partenza + datetime.timedelta(days=categoria.tempo_standard)
                result = {'value': {
...
class attivita_storico(osv.Model):
    ...
    _columns = {
        ...
        'data_evento': fields.datetime('Data Evento', readonly=True, required=True),
...
class res_users(osv.Model):
    ...
    _columns = {
        'is_visible': fields.function(_is_visible, fnct_search=_is_visible_search, type="boolean", method=True, string='Assegnatario'),
