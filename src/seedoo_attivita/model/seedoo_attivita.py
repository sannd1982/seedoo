...
class protocollo_protocollo(orm.Model):
    ...
    _columns = {
        'attivita_ids': fields.one2many('attivita.attivita', 'protocollo_id',
                                        'Attivita', readonly=True),
    }

    def write(self, cr, uid, ids, data, context=None):
        res_id = super(protocollo_protocollo, self).write(cr, uid, ids, data,
                                                          context=context)
        ...
        if data.has_key('classification') and prot.state != 'draft':
            ...
            for attivita in prot.attivita_ids:
                if attivita.template_instance and attivita.state in [
            ...
            if prot.classification and len(
                    prot.classification.attivita_ids) > 0:
                for attivita_titolario in prot.classification.attivita_ids:
                    ...
                    activity_vals = {
                        'name': "Protocollo num %s - " % (
                            prot.name) + attivita_titolario.name,
                    ...
                    attivita_obj.create(cr, SUPERUSER_ID, activity_vals,
                                        context=None)
                    res_id = self.write(cr, uid, ids, {'assigne_users': [
                        (4, attivita_titolario.assegnatario_id.id)]})
        return res_id
        ...
        for prot in self.browse(cr, uid, ids):
            ...
            if prot.assigne:
                for d in prot.assigne:
                    department_manager = self.pool.get("hr.employee").browse(
                        cr, uid, d.manager_id.id)
                    r = self.pool.get('resource.resource').browse(cr, uid,
                                                                  department_manager.resource_id.id)
                    if r:
            ...
            for u in list(set(user_list)):
                ...
                activity_vals = {
                    'name': "Assegnazione protocollo num %s a %s " % (
                        prot.name, user.partner_id.name),
                ...
                self.pool.get("attivita.attivita").create(cr, SUPERUSER_ID,
                                                          activity_vals,
                                                          context=None)
            if prot.classification and len(
                    prot.classification.attivita_ids) > 0:
                ...
                for attivita_titolario in prot.classification.attivita_ids:
                    ...
                    assegnatari_list.append(
                        attivita_titolario.assegnatario_id.id)
                    self.pool.get("attivita.attivita").create(cr, SUPERUSER_ID,
                                                              activity_vals,
                                                              context=None)
                    self.write(cr, uid, ids, {'assigne_users': [
                        (4, attivita_titolario.assegnatario_id.id)]})
        return True
